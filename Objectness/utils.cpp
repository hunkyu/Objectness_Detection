...
void PCA_output(PCA_Out_Result* &result, vector<cv::Mat>& InImg, vector<int>& InImgIdx, int PatchSize, int NumFilters, cv::Mat& Filters, int threadnum){
	
	//PCA_Out_Result* 
	result = new PCA_Out_Result;
...
PCA_Train_Result* PCANet_train(vector<cv::Mat>& InImg, PCANet* PcaNet, bool is_extract_feature){
	...
	for(int s=0; s<PcaNet->NumStages; s++){
		...
		if(s != PcaNet->NumStages - 1){
			/*temp =*/ PCA_output(temp, out_result->OutImg, out_result->OutImgIdx, PcaNet->PatchSize,
...
# pragma omp parallel for default(none) num_threads(coreNum) private(temp, hashing_r) shared(features, out_result, PcaNet, first, last, outIdx_length, img_length, train_result, end)
		for(int i=0; i<img_length; i++){
			...
			/*temp =*/ PCA_output(temp,subInImg, subIdx, PcaNet->PatchSize,
								PcaNet->NumFilters[end], train_result->Filters[end], 2);
			
			/*hashing_r = */HashingHist(hashing_r, PcaNet, temp->OutImgIdx, temp->OutImg);
...
void HashingHist(Hashing_Result* &ha_result, PCANet* PcaNet, vector<int>& ImgIdx, vector<cv::Mat>& Imgs){

	ha_result = new Hashing_Result;
